...
namespace Database
{
    ...
    public class DbRowList<T> where T : DbRow, new()
    {
        ...
        public bool IsContains(DbRow outRow)
        {
            List<int> indexes = outRow.GetPrimaryIndexes();
            object[] outValues = outRow.GetValues();

            foreach (T inRow in _rows)
            {
                if (inRow.Same(outRow))
                {
                    return true;
                }
            }
